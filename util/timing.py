...
def time(facade, name, runs, *args, func=None):
    ...
    bench_args = facade.adapt_benchmark_args(args)
    ...
    if func is not None:
        funcs = [f for f in dir(facade) if f == func]

    log.debug(f"Benchmark funcs {funcs}")

    g = {"bench_args": bench_args, "facade": facade}
